...
from .models import DocumentFormat, DocumentGenerationResponse, UserQuery
...
class DocumentGenerator:
    ...
    async def generate_document(self, query: UserQuery) -> DocumentGenerationResponse:
        ...
        try:
            logger.info(
                f"Generating document with format: {query.format} ({query.format.value})"
            )

            # Generate content using LLM
            content = await self._generate_content_with_llm(query)
            logger.info(
                f"Generated content length: {len(content)} chars, starts with: {content[:100]}..."
            )

            # Convert to desired format if needed
            final_content = await self._convert_format(content, query.format)
            logger.info(
                f"Final content length: {len(final_content)} chars, starts with: {final_content[:100]}..."
            )
...
- Ensure the content thoroughly addresses the user's question"""
    ...
    async def _convert_format(self, content: str, target_format: DocumentFormat) -> str:
        ...
        if target_format == DocumentFormat.HTML and content.strip().startswith(
            "<!DOCTYPE html>"
        ):
            return content
        elif (
            target_format == DocumentFormat.MARKDOWN
            and not content.strip().startswith("<!DOCTYPE html>")
        ):
            return content
        ...
        if target_format == DocumentFormat.HTML:
            ...
            converted = await self.mcp_manager.convert_with_pandoc(
                content, "markdown", "html5"
            )
            if converted:
...
{converted}
        ...
        try:
            response = await self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "Generate a concise, descriptive title (maximum 10 words) for a document that answers the following question. Respond with only the title, no additional text.",
                    },
    ...
    async def _save_document(
        self, content: str, title: str, format: DocumentFormat
    ) -> Path | None:
